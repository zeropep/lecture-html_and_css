...
class Items{   
	...
	function read(){	
		if($this->id) {
			...
			$stmt = $this->conn->prepare("SELECT * FROM ".$this->itemsTable." ORDER BY id DESC");		
		}		
		$stmt->execute();			
	...
	function update(){
		...
		$stmt = $this->conn->prepare("
			...
			SET name= ?, description = ?, price = ?, category_id = ?, modified = ?
			WHERE id = ?");
		...
		$stmt->bind_param("ssiisi", $this->name, $this->description, $this->price, $this->category_id, $this->modified, $this->id);
